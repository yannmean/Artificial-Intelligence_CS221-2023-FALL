...
def find_alphabetically_first_word(text: str) -> str:
    ...
    Create doctests
    >>> find_alphabetically_first_word('the word that would come first')
    {'come'}
    >>> find_alphabetically_first_word('If the input text is')
    {'If'}
    """
...
def mutate_sentences(sentence: str) -> List[str]:
    ...
    for i in range(len(mots) - 1):
        dic[mots[i]].append(mots[i + 1])

    # now generate the list of permutated sentences

    sentences = []

    def add_mot(length, current_sentence):
        temp = current_sentence.split()
        # if sentence_length<len(temp)+1:return
        if length == len(temp) + 1:
            for mot in dic[temp[-1]]:
                sentences.append(current_sentence + ' ' + mot)
            return
        if temp[-1] in dic:
            for mot in dic[temp[-1]]:
                add_mot(length, current_sentence + ' ' + mot)

    for mot in list(dic):
        add_mot(len(mots), mot)
...
def find_nonsingleton_words(text: str) -> Set[str]:
    ...
    for mot in mots:
        if not mot in dic:
            dic[mot] = 0
        dic[mot] += 1
    return set([mot for mot, count in dic.items() if count > 1])
    # END_YOUR_CODE
